...
if (isset($_SESSION['user_name'])) {
    ...
    logAction($conn, $member_name, 'User logged in');
}
                ...
                <span class="navbar-text me-3">
                    <?php
                        if (isset($_SESSION['full_name'])) {
                            echo htmlspecialchars("Hello ".$_SESSION['full_name']);
                        } else {
                            echo 'Guest'; 
                        }
                    ?>
                ...
                <tbody>
                    <?php
                        ...
                        if ($user) {
                            ...
                            echo '<td>';
                        echo '<span style="background-color: ' . htmlspecialchars($user['color']) . '; display: inline-block; width: 20px; height: 20px; border-radius: 50%;"></span>';
                        echo '</td>';
                            
                            // 生成包含 full_name 的编辑連接
                            echo '<td>';
                            echo '<a href="edit.php?full_name=' . urlencode($user['full_name']) . '" class="btn btn-primary">Edit</a>';
                            echo '</td>';
                            echo '</tr>';
                        } else {
            ...
            fetch('get_message.php')
                ...
                .then(data => {
                    ...
                    messagesList.innerHTML = data.messages.map(message => {
                        ...
                        return `
                            <div class="message" data-id="${message.id}" style="margin-bottom: 20px;">
                                <h4>${message.title}</h4>
                                <p>${message.content}</p>
                                ...
                                <div class="replies" style="margin-top: 10px;">
                                    ${message.replies.length ? message.replies.map(reply => {
                                        ...
                                        return `
                                            <div class="reply" data-id="${reply.id}" style="margin-top: 10px;">
                                                <p>${reply.content}</p>
        ...
        window.editMessage = function(messageId) {
            ...
            if (newTitle && newContent) {
                fetch('edit_message.php', {
                    ...
                    body: JSON.stringify({ message_id: messageId, title: newTitle, content: newContent })
                })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
